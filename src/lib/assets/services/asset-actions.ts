...
async function submitGlobalAssetGenerateTask(input: AssetGenerateInput) {
  ...
  const dedupeKey = `${TASK_TYPE.ASSET_HUB_IMAGE}:${targetType}:${input.assetId}:${normalizedKind === 'character' ? appearanceIndex : 'na'}:${toNumber(input.body.imageIndex) === null ? count : `single:${toNumber(input.body.imageIndex)}`}`
